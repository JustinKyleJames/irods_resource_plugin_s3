    ...
    bool ignore_not_found_error )
{
    ...
    *pStatus = status;
    ...
    if( status != S3StatusOK ) {
        s3_logger::error( "  S3Status: [{}] - {}", S3_get_status_name( status ), (int) status );
        s3_logger::error( "    S3Host: {}", pCtx->hostName );
    }
    if (status != S3StatusOK && function )
        s3_logger::error( "  Function: {}", function );
    if (error && error->message)
        s3_logger::error( "  Message: {}", error->message);
    if (error && error->resource)
        s3_logger::error( "  Resource: {}", error->resource);
    if (error && error->furtherDetails)
        s3_logger::error( "  Further Details: {}", error->furtherDetails);
    if (error && error->extraDetailsCount) {
        s3_logger::error( "{}", "  Extra Details:");

        for (i = 0; i < error->extraDetailsCount; i++) {
            s3_logger::error( "    {}: {}", error->extraDetails[i].name, error->extraDetails[i].value);
        }
    }
}
...
std::int64_t s3GetMaxUploadSizeMB (irods::plugin_property_map& _prop_map)
{
    ...
    if (ret.ok()) {
        try {
            ...
            std::string resource_name = get_resource_name(_prop_map);
            s3_logger::error(
                "[resource_name={}] failed to cast {} [{}] to a std::int64_t", resource_name,
                max_size_str, s3_max_upload_size_mb);
        }
    } else {
        ...
        if (ret.ok()) {
            std::string resource_name = get_resource_name(_prop_map);
            s3_logger::warn("[resource_name={} - {} is deprecated.  Use {}",
                    resource_name, s3_max_upload_size, s3_max_upload_size_mb);
            ...
            try {
                ...
                s3_logger::error(
                    "[resource_name={}] failed to cast {} [{}] to a std::int64_t", resource_name,
                    max_size_str, s3_max_upload_size);
            }
        }
    }
...
std::size_t get_max_retry_wait_time_sec(irods::plugin_property_map& _prop_map) {
    ...
    if( ret.ok() ) {
        ...
        std::string resource_name = get_resource_name(_prop_map);
        if( ret.ok() ) {
            s3_logger::warn("[resource_name={} - {} is being deprecated.  Use {}",
