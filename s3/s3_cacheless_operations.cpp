...
rodsLog(LOG_ERROR, "%s:%d ----------------- ", __FUNCTION__, __LINE__);
        ...
        // TODO changed last arg to true 
        if(0 > (retVal = ent->Read(static_cast<char*>(_buf), offset, _len, true))){
...
rodsLog(LOG_ERROR, "%s:%d ----------------- ", __FUNCTION__, __LINE__);
        ...
        if(ent->GetFd() != static_cast<int>(fd)){
            S3FS_PRN_WARN("different fd(%d - %llu)", ent->GetFd(), (unsigned long long)(fd));
        }
...
rodsLog(LOG_ERROR, "%s:%d ----------------- ", __FUNCTION__, __LINE__);
        ...
        S3FS_PRN_INFO("[from=%s][to=%s]", from.c_str(), _new_file_name);
