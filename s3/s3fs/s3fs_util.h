...
class AutoLockInterprocessResursiveMutex
{
  ...
  public:
    explicit AutoLockInterprocessResursiveMutex(boost::interprocess::interprocess_recursive_mutex *mutex, bool no_wait = false);
    bool isLockAcquired() const;
    ~AutoLockInterprocessResursiveMutex();
};
