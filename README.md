...
-   `CIRCULAR_BUFFER_SIZE` - The plugin uses a circular buffer to store data while it is being streamed to S3.  The size of the circular buffer is CIRCULAR_BUFFER_SIZE * S3_MPU_CHUNK.  The default value is 4 so if the S3_MPU_CHUNK is the default of 5 MiB the circular buffer size will be 20 MiB.  CIRCULAR_BUFFER_SIZE must be at least 2.  If a size is set lower than 2 then it will default to 2.  AWS S3 multipart uploads are limited to 10,000 parts, so for a sufficiently large object the plugin will automatically grow the part size beyond the configured circular buffer size to stay within that limit.  When this happens, the circular buffer is drained as data is streamed to S3 rather than only after a whole part completes, so a failed part cannot be retried (the whole upload must be retried).  An object is rejected outright at open time if it is too large to be represented by S3 multipart upload at all, i.e. it would require a part size larger than the configured `S3_MAX_UPLOAD_SIZE_MB` even when using all 10,000 parts.  `S3_MAX_UPLOAD_SIZE_MB` defaults to 5 GiB (AWS's real per-part maximum), giving a rough ceiling of 48 TiB or more, depending on the number of parallel transfer threads; it may be raised for non-AWS S3-compatible endpoints that support larger parts.
