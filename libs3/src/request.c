...
static void request_headers_done(Request* request)
{
	...
	if (curl_easy_getinfo(request->curl, CURLINFO_RESPONSE_CODE, &httpResponseCode) != CURLE_OK) {
	...
	response_headers_handler_done(&(request->responseHeadersHandler), request->curl);
	...
	if (request->propertiesCallback && (request->httpResponseCode >= 200) && (request->httpResponseCode <= 299)) {
		request->status = (*(request->propertiesCallback))(
			&(request->responseHeadersHandler.responseProperties), request->callbackData);
	}
}
...
static size_t curl_write_func(void* ptr, size_t size, size_t nmemb, void* data)
{
	...
	if ((request->httpResponseCode < 200) || (request->httpResponseCode > 299)) {
                                  ...
                                  const char* headerValue)
{
	...
	if (addPrefix) {
		snprintf(headerNameWithPrefix, sizeof(headerNameWithPrefix), S3_METADATA_HEADER_NAME_PREFIX "%s", headerName);
                                    ...
                                    RequestComputedValues* values)
{
	...
	if (properties) {
		...
		if (properties->useServerSideEncryption) {
			append_amz_header(values, 0, "x-amz-server-side-encryption", "AES256");
		}
	...
	if (params->httpRequestType == HttpRequestTypeCOPY) {
		...
		if (params->copySourceBucketName && params->copySourceBucketName[0] && params->copySourceKey &&
		    params->copySourceKey[0])
		{
			char bucketKey[S3_MAX_METADATA_SIZE];
			snprintf(bucketKey, sizeof(bucketKey), "/%s/%s", params->copySourceBucketName, params->copySourceKey);
	...
	if (params->bucketContext.securityToken) {
		append_amz_header(values, 0, "x-amz-security-token", params->bucketContext.securityToken);
	}

	if (!forceUnsignedPayload &&
	    (params->httpRequestType == HttpRequestTypeGET || params->httpRequestType == HttpRequestTypeCOPY ||
	     params->httpRequestType == HttpRequestTypeDELETE || params->httpRequestType == HttpRequestTypeHEAD))
	{
		// empty payload
...
static S3Status compose_standard_headers(const RequestParams* params, RequestComputedValues* values)
{
#define do_put_header(fmt, sourceField, destField, badError, tooLongError)                                          \
	do {                                                                                                            \
		if (params->putProperties && params->putProperties->sourceField && params->putProperties->sourceField[0]) { \
			...
			int len = snprintf(values->destField, sizeof(values->destField), fmt, val);                             \
...
#define do_get_header(fmt, sourceField, destField, badError, tooLongError)                                          \
	do {                                                                                                            \
		if (params->getConditions && params->getConditions->sourceField && params->getConditions->sourceField[0]) { \
			/* Skip whitespace at beginning of val */                                                               \
	...
	if (params->bucketContext.uriStyle == S3UriStyleVirtualHost) {
		...
		size_t len = snprintf(values->hostHeader,
	...
	// Cache-Control
	do_put_header(
		"Cache-Control: %s", cacheControl, cacheControlHeader, S3StatusBadCacheControl, S3StatusCacheControlTooLong);

	// ContentType
	do_put_header(
		"Content-Type: %s", contentType, contentTypeHeader, S3StatusBadContentType, S3StatusContentTypeTooLong);

	// MD5
	do_put_header("Content-MD5: %s", md5, md5Header, S3StatusBadMD5, S3StatusMD5TooLong);
	...
	// ContentEncoding
	do_put_header("Content-Encoding: %s",
	              contentEncoding,
	...
	if (params->putProperties && (params->putProperties->expires >= 0)) {
		...
		struct tm gmt;
		strftime(values->expiresHeader,
		         sizeof(values->expiresHeader),
	...
	// If-Modified-Since
	if (params->getConditions && (params->getConditions->ifModifiedSince >= 0)) {
	...
	// If-Unmodified-Since header
	if (params->getConditions && (params->getConditions->ifNotModifiedSince >= 0)) {
	...
	// If-Match header
	do_get_header("If-Match: %s", ifMatchETag, ifMatchHeader, S3StatusBadIfMatchETag, S3StatusIfMatchETagTooLong);

	// If-None-Match header
	do_get_header("If-None-Match: %s",
	              ifNotMatchETag,
	              ifNoneMatchHeader,
	...
	if (params->startByte || params->byteCount) {
		if (params->byteCount) {
			snprintf(values->rangeHeader,
			         sizeof(values->rangeHeader),
			         "Range: bytes=%llu-%llu",
...
static S3Status encode_key(const RequestParams* params, RequestComputedValues* values)
{
...
static void canonicalize_signature_headers(RequestComputedValues* values)
{
	...
	memcpy(sortedHeaders, values->amzHeaders, (values->amzHeadersCount * sizeof(sortedHeaders[0])));
...
static void sort_query_string(const char* queryString, char* result, unsigned int result_size)
{
...
#define buf_append(buf, bufLen, format, ...) len += snprintf(&(buf[len]), bufLen - len, format, __VA_ARGS__)
	...
	char scope[sizeof(values->requestDateISO8601) + sizeof(awsRegion) + sizeof("//s3/aws4_request") + 1];
	snprintf(scope, sizeof(scope), "%.8s/%s/s3/aws4_request", values->requestDateISO8601, awsRegion);

	char stringToSign[17 + 17 + sizeof(values->requestDateISO8601) + sizeof(scope) + sizeof(canonicalRequestHashHex) +
	                  1];
	snprintf(stringToSign,
	         sizeof(stringToSign),
	         "AWS4-HMAC-SHA256\n%s\n%s\n%s",
	         values->requestDateISO8601,
	         scope,
	...
	CCHmac(kCCHmacAlgSHA256, accessKey, strlen(accessKey), values->requestDateISO8601, 8, dateKey);
	unsigned char dateRegionKey[S3_SHA256_DIGEST_LENGTH];
	CCHmac(kCCHmacAlgSHA256, dateKey, S3_SHA256_DIGEST_LENGTH, awsRegion, strlen(awsRegion), dateRegionKey);
	unsigned char dateRegionServiceKey[S3_SHA256_DIGEST_LENGTH];
	CCHmac(kCCHmacAlgSHA256, dateRegionKey, S3_SHA256_DIGEST_LENGTH, "s3", 2, dateRegionServiceKey);
	unsigned char signingKey[S3_SHA256_DIGEST_LENGTH];
	CCHmac(kCCHmacAlgSHA256,
	       dateRegionServiceKey,
	       S3_SHA256_DIGEST_LENGTH,
	       "aws4_request",
	       strlen("aws4_request"),
	       signingKey);

	unsigned char finalSignature[S3_SHA256_DIGEST_LENGTH];
	CCHmac(kCCHmacAlgSHA256, signingKey, S3_SHA256_DIGEST_LENGTH, stringToSign, strlen(stringToSign), finalSignature);
#else
	const EVP_MD* sha256evp = EVP_sha256();
	unsigned char dateKey[S3_SHA256_DIGEST_LENGTH];
	HMAC(sha256evp, accessKey, strlen(accessKey), (const unsigned char*) values->requestDateISO8601, 8, dateKey, NULL);
	unsigned char dateRegionKey[S3_SHA256_DIGEST_LENGTH];
	HMAC(sha256evp,
	     dateKey,
	     S3_SHA256_DIGEST_LENGTH,
	     (const unsigned char*) awsRegion,
	     strlen(awsRegion),
	     dateRegionKey,
	     NULL);
	unsigned char dateRegionServiceKey[S3_SHA256_DIGEST_LENGTH];
	HMAC(sha256evp, dateRegionKey, S3_SHA256_DIGEST_LENGTH, (const unsigned char*) "s3", 2, dateRegionServiceKey, NULL);
	unsigned char signingKey[S3_SHA256_DIGEST_LENGTH];
	HMAC(sha256evp,
	     dateRegionServiceKey,
	     S3_SHA256_DIGEST_LENGTH,
...
static S3Status compose_uri(char* buffer,
                            int bufferSize,
...
#define uri_append(fmt, ...)                                                 \
	...
	const char* hostName = bucketContext->hostName ? bucketContext->hostName : defaultHostNameG;

	if (bucketContext->bucketName && bucketContext->bucketName[0]) {
	...
	if (queryParams) {
		uri_append("%s%s", (subResource && subResource[0]) ? "&" : "?", queryParams);
	}
...
static S3Status setup_curl(Request* request, const RequestParams* params, const RequestComputedValues* values)
{
	CURLcode status;
                            ...
                            Request** reqReturn)
{
	...
	// Compute the URL
	if ((status = compose_uri(request->uri,
	                          sizeof(request->uri),
	                          &(params->bucketContext),
	                          values->urlEncodedKey,
	                          params->subResource,
	                          params->queryParams)) != S3StatusOK)
	{
...
S3Status request_api_initialize(const char* userAgentInfo, int flags, const char* defaultHostName)
{
	if (curl_global_init(CURL_GLOBAL_ALL & ~((flags & S3_INIT_WINSOCK) ? 0 : CURL_GLOBAL_WIN32)) != CURLE_OK) {
	...
	if (snprintf(defaultHostNameG, S3_MAX_HOSTNAME_SIZE, "%s", defaultHostName) >= S3_MAX_HOSTNAME_SIZE) {
		return S3StatusUriTooLong;
	}
	...
	if (uname(&utsn)) {
		...
		snprintf(platform, sizeof(platform), "%s%s%s", utsn.sysname, utsn.machine[0] ? " " : "", utsn.machine);
	}
...
static S3Status setup_request(const RequestParams* params, RequestComputedValues* computed, int forceUnsignedPayload)
{
	S3Status status;

	// Validate the bucket name
	if (params->bucketContext.bucketName &&
	    ((status = S3_validate_bucket_name(params->bucketContext.bucketName, params->bucketContext.uriStyle)) !=
	     S3StatusOK))
	{
		return status;
	}
	...
	struct tm gmt;
	gmtime_r(&now, &gmt);
	strftime(computed->requestDateISO8601, sizeof(computed->requestDateISO8601), "%Y%m%dT%H%M%SZ", &gmt);
	...
	canonicalize_resource(
		&params->bucketContext, computed->urlEncodedKey, computed->canonicalURI, sizeof(computed->canonicalURI));
	canonicalize_query_string(params->queryParams,
	                          params->subResource,
...
void request_finish(Request* request)
{
	...
	if (request->status == S3StatusOK) {
		...
		// detailing the error
		if ((request->status == S3StatusOK) && ((request->httpResponseCode < 200) || (request->httpResponseCode > 299)))
		{
	...
	(*(request->completeCallback))(request->status, &(request->errorParser.s3ErrorDetails), request->callbackData);
...
#define MAX_EXPIRES 604800
	...
	RequestParams params = {http_request_method_to_type(httpMethod),
	                        *bucketContext,
	                        key,
	                        NULL,
	                        resource,
	...
	char queryParams[sizeof("X-Amz-Algorithm=AWS4-HMAC-SHA256") + sizeof("&X-Amz-Credential=") +
	                 sizeof(computed.authCredential) + sizeof("&X-Amz-Date=") + sizeof(computed.requestDateISO8601) +
	                 sizeof("&X-Amz-Expires=") + 64 + sizeof("&X-Amz-SignedHeaders=") + sizeof(computed.signedHeaders) +
	                 sizeof("&X-Amz-Signature=") + sizeof(computed.requestSignatureHex) + 1];
	snprintf(queryParams,
	         sizeof(queryParams),
	         "X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=%s"
	         "&X-Amz-Date=%s&X-Amz-Expires=%d"
	         "&X-Amz-SignedHeaders=%s&X-Amz-Signature=%s",
	         computed.authCredential,
	         computed.requestDateISO8601,
	         expires,
	         computed.signedHeaders,
	         computed.requestSignatureHex);

	return compose_uri(
		buffer, S3_MAX_AUTHENTICATED_QUERY_STRING_SIZE, bucketContext, computed.urlEncodedKey, resource, queryParams);
}
