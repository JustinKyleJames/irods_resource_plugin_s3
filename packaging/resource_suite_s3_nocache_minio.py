...
class ResourceBase_Minio(session.make_sessions_mixin([('otherrods', 'rods')], [('alice', 'apass'), ('bobby', 'bpass')])):

    def setUp(self):
        super(ResourceBase_Minio, self).setUp()
        ...
        self.s3_context = "S3_DEFAULT_HOSTNAME=%s;S3_AUTH_FILE=%s;S3_REGIONNAME=%s;S3_RETRY_COUNT=3;S3_WAIT_TIME_SEC=2;S3_PROTO=HTTP;ARCHIVE_NAMING_POLICY=consistent;HOST_MODE=cacheless_attached;S3_ENABLE_MD5=1;S3_ENABLE_MPU=%d;S3_SIGNATURE_VERSION=%d" % (self.s3endPoint, self.keypairfile, self.s3region, self.s3EnableMPU, self.s3signature_version)
    ...
    def tearDown(self):
        ...
        super(ResourceBase_Minio, self).tearDown()
...
class ResourceSuite_S3_NoCache_Minio(ResourceBase_Minio):
...
class Test_S3_NoCache_Base_Minio(ResourceSuite_S3_NoCache_Minio):
...
OUTPUT ruleExecOut
    ...
    def test_detached_mode(self):
        ...
        s3_context_detached = "S3_DEFAULT_HOSTNAME=%s;S3_AUTH_FILE=%s;S3_REGIONNAME=%s;S3_RETRY_COUNT=3;S3_WAIT_TIME_SEC=2;S3_PROTO=HTTP;ARCHIVE_NAMING_POLICY=consistent;HOST_MODE=cacheless_detached;S3_ENABLE_MD5=1;S3_ENABLE_MPU=%d;S3_SIGNATURE_VERSION=%d" % (self.s3endPoint, self.keypairfile, self.s3region, self.s3EnableMPU, self.s3signature_version)
