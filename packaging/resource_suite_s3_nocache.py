...
OUTPUT ruleExecOut
    ...
    def test_iput_fails_cleanly_when_object_needs_too_many_parts__issue_2234(self):
        try:
            ...
            self.admin.assert_icommand(f'iadmin mkresc s3resc1 s3 {hostname}:/{self.s3bucketname}/{hostuser}/s3resc1 {s3_context}', 'STDOUT_SINGLELINE', 'Creating')
            ...
            if self.s3EnableMPU:
                ...
                expected_message = 'cannot be uploaded because S3 multipart upload is disabled'
            ...
            out, err, rc = self.user0.run_icommand(['iput', '-N', '1', '-f', '-R', 's3resc1', file1])
    ...
    def test_iput_fails_cleanly_when_thread_split_pushes_object_past_max_parts__issue_2234(self):
        try:
            ...
            self.admin.assert_icommand(f'iadmin mkresc s3resc2 s3 {hostname}:/{self.s3bucketname}/{hostuser}/s3resc2 {s3_context}', 'STDOUT_SINGLELINE', 'Creating')
            ...
            # (from a byte-count mismatch) rather asking the server for the underlying
            ...
            out, err, rc = self.user0.run_icommand(
                    ['iput', '-N', str(number_of_threads), '-f', '-R', 's3resc2', file1])
            self.assertNotEqual(0, rc)
            self.assertIn(expected_error, err)
...
class Test_S3_NoCache_Large_File_Tests_Base(Test_S3_NoCache_Base):
    ...
    def test_iput_with_N_flag__issue_2198(self):
        ...
        try:
            ...
            for file_size in file_sizes:
                ...
                # put the file
                self.user1.assert_icommand(f'iput {filename}')
                self.user1.assert_icommand(f'iget -f {filename} {get_filename}')
                assert_command(['diff', '-q', filename, get_filename])

                for n_value in [0, 1, 2, 3, 4, 8]:
                    self.user1.assert_icommand(f'iput -f -N{n_value} {filename}')
                    self.user1.assert_icommand(f'iget -f {filename} {get_filename}')
                    assert_command(['diff', '-q', filename, get_filename])
                ...
                self.user1.assert_icommand(f'irm -f {filename}')
    ...
    def test_recursive_put_small_files_with_checksum_and_N_flag__issue_2198(self):
        ...
        dirname = f'{inspect.currentframe().f_code.co_name}'
        filename1 = f'{inspect.currentframe().f_code.co_name}1'
        get_filename1 = f'{filename1}.get'
        filename2 = f'{inspect.currentframe().f_code.co_name}2'
        get_filename2 = f'{filename2}.get'
        filename3 = f'{inspect.currentframe().f_code.co_name}3'
        get_filename3 = f'{filename3}.get'
        ...
        try:
            ...
            # create the files
            lib.make_arbitrary_file(f'{dirname}/{filename1}', 100)
            lib.make_arbitrary_file(f'{dirname}/{filename2}', 100)
            lib.make_arbitrary_file(f'{dirname}/{filename3}', 100)

            # recursively put the directory
            self.user1.assert_icommand(f'iput -r -N2 -K {dirname}')

            #verify the files are in iRODS 
            self.user1.assert_icommand(f'ils -l {dirname}', 'STDOUT_MULTILINE', [filename1, filename2, filename3])

            # get and verify the file contents 
            self.user1.assert_icommand(f'iget {dirname}/{filename1} {get_filename1}')
            self.user1.assert_icommand(f'iget {dirname}/{filename2} {get_filename2}')
            self.user1.assert_icommand(f'iget {dirname}/{filename3} {get_filename3}')
            assert_command(['diff', '-q', f'{dirname}/{filename1}', get_filename1])
            assert_command(['diff', '-q', f'{dirname}/{filename2}', get_filename2])
            assert_command(['diff', '-q', f'{dirname}/{filename3}', get_filename3])

        finally:
            # clean up whatever was not cleaned up
            self.user1.assert_icommand(f'irm -rf {dirname}')
    ...
    def test_multiple_irsync_to_s3__issue_2204(self):
        ...
        try:
            # create 
            lib.make_arbitrary_file(f'{filename1}', 100*1024*1024)

            # put filename1 
            self.user1.assert_icommand(f'iput -K {filename1}')

            # irsync to filename2
            self.user1.assert_icommand(f'irsync -K i:{filename1} i:{filename2}')

            # verify the files are in iRODS 
            self.user1.assert_icommand(f'ils -l', 'STDOUT_MULTILINE', [filename1, filename2])

            # get filename1 and filename2
            self.user1.assert_icommand(f'iget {filename1} {get_filename1}')
            self.user1.assert_icommand(f'iget {filename2} {get_filename2}')
            ...
            self.user1.assert_icommand(f'irm -f {filename2}')
            s3plugin_lib.remove_if_exists(get_filename1)
            s3plugin_lib.remove_if_exists(get_filename2)
            ...
            self.user1.assert_icommand(f'irm -rf {filename2}')
    ...
    def test_multiple_irsync_to_s3_with_N_flag__issue_2204(self):
        ...
        try:
            ...
            # irsync to filename2
            self.user1.assert_icommand(f'irsync -N3 -K i:{filename1} i:{filename2}')
